...
- **Architecture** follows the
  ...
  `metrics.py`, `montage.py`, `charts.py`, `transfer.py`, `resample.py`,
  `smooth.py`) is Django-free and fully typed. `resample.py` and `smooth.py`
  are the two modules the deployment does not carry the dependencies for —
  they need scipy, which lives in the `render` pixi feature rather than in the
  container image, because rendering only ever happens where the niftis are.
  `services.py` therefore imports `resample` inside `_staged_render`, and
  `montage._render_one` imports `smooth` inside the render worker, rather than
  at module scope, so importing `services` on the server still works.
  `lake.py` is the one module that talks to a
...
- **Montage files** are ordinary Django media under `MEDIA_ROOT`: written
  ...
  *layout*, `runs/<Run.uuid>/<Run.montage_digest>/ic007_func_raw_axial.avif`. The uuid
...
- **A montage name is rebuilt, never accepted.** Everything arriving from
  ...
  `montage.montage_name`, from a parsed integer, a `BACKGROUNDS` key, a
  `SMOOTHINGS` key, an `AXES` key, and a format sniffed from the bytes — so no
  string a client chose reaches a storage path.
