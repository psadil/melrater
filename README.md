...
Montage rendering parallelizes across `--workers` processes (default: CPUs − 2; a 96-component run takes about 20 seconds).
...
Each component is also rendered at two smoothing levels: the z-map as MELODIC wrote it, and the same map smoothed with a 9.4 mm FWHM Gaussian inside the brain mask and rescaled so its bulk is unit scale again. That is the display Griffanti et al. (2017, [doi:10.1016/j.neuroimage.2016.12.036](https://doi.org/10.1016/j.neuroimage.2016.12.036)) recommend for data that were not smoothed before ICA, and the kernel is theirs: unsmoothed maps carry many small scattered clusters whether or not a component is noise, and the smoothed map is where "a low number of large clusters" — or its absence — can be read off. The unsmoothed map stays because it is where a multiband checkerboard, an alternating-sign stripe, and the exact voxel a peak sits in are visible. Montages are encoded at voxel resolution and upscaled by the browser, which is what pays for the second level: a registered 96-component run is ~1,152 files and ~66 MB, against ~61 MB before. The slice numbers are not in the image at all — the page draws them from the run's recorded slice picks — so they stay sharp at any zoom.
...
Keyboard shortcuts on a component page: `1`/`s`, `2`/`u`, `3`/`n` to rate, `←`/`→` to move between components, `g` to jump to one by number, `a` to toggle auto-advance, `o` to cycle the montage plane, `b` to swap the montage background, and `m` to swap between the unsmoothed and the smoothed map. The `?` beside the rating buttons is Griffanti et al.'s decision procedure — spatial map, then time series, then spectrum; keep what is not clearly noise — with the look-alikes to watch for.

On `m`: the smoothed map is per-component rescaled, so read cluster count, size and placement rather than absolute colour. It is the view for a network's shape, a vein along the sinus, CSF in the ventricles or a white-matter pattern; the unsmoothed map is the view for stripes, checkerboards, the mixed signs of a susceptibility artefact and where a peak actually sits.
...
**Migration note.** Montage filenames gained a smoothing token — `ic007_func_axial.avif` became `ic007_func_raw_axial.avif` (as they had earlier gained the background token) — which changes every montage digest. Any run ingested before that change serves 404s until it is re-rendered and pushed:
...
40 GB is likely enough. The arithmetic on 40: Ubuntu and Docker take ~3 GB; the image is 2.43 GB *unpacked* (the 535 MiB under [Without a registry](#without-a-registry) is the wire size, a different measurement) and a redeploy holds two of those until `docker image prune -f`; an incoming push holds one run's tar in `/tmp` while it is read, which is tens of megabytes. That leaves roughly 16 GB of montages — with both backgrounds and both smoothing levels, about 240 runs, and it is the binding constraint on how many runs the box can hold. Skip the external volume for the same reason it is a chore later: `compose.yaml` bind-mounts absolute paths, so a volume added afterwards has to be mounted at `/srv/melrater` itself — stack stopped, data moved, everything re-chowned.
...
- **The tar ceiling exists in two repos.** `MELRATER_INGEST_MAX_TAR_BYTES` (`settings.py`) is the app's limit; `request_body @api { max_size }` in the proxy repo's `Caddyfile` is the edge's, and it has to stay *above* the app's so melrater's own refusal — which says what was wrong — is what a client sees. Nothing checks them against each other. Too low a value at the edge surfaces as a bare `HTTP 413:` with an empty body, on the largest runs only, with nothing in melrater's logs: the request never reached it. A run's tar is every montage for every background and smoothing level, so it grew with the second background; rendering at voxel resolution rather than 2× paid for the smoothed level (~66 MB for a registered 96-component run, ~176 MB at the 256-component cap), and a third of either would grow it again.
